...
// **FIX:** The API uses 'option' (singular) and the value can be null.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const subjectMatch = queryParts.find((p: string) =>
      ["chemistry", "physics", "biology", "mathematics", "english"].includes(p)
    );
    ...
    if (retrievedQuestion) {
      ...
      // **FIX: Use the correct 'option' property and handle null values.**
      ...
      3. Most importantly, give a detailed, step-by-step explanation of the underlying concept. **If possible, try to relate the explanation to one of the student's interests to make it more engaging.** For example, if the interest is 'football' and the topic is 'physics', you could use an analogy of a kicking a ball.
      ...
      prompt = `You are a world-class WAEC tutor AI with a live database of past questions. The user sent a message: "${userQuery}".
